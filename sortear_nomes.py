...
def carregar_categorias(caminho_csv: Path) -> dict[str, list[str]]:
    ...
    with caminho_csv.open("r", encoding="utf-8-sig", newline="") as arquivo:
        ...
        if not leitor.fieldnames or len(leitor.fieldnames) < 1:
            raise ValueError("O arquivo CSV deve ter ao menos 1 coluna de categoria.")
...
def criar_fila_embaralhada(nomes: list[str], gerador: random.Random, ultimo: str | None = None) -> list[str]:
    fila = nomes[:]
    gerador.shuffle(fila)
    # Evita repetição imediata
    if ultimo and len(fila) > 1 and fila[0] == ultimo:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Sorteia nomes de uma tabela com qualquer número de categorias, "
            "mantendo cada nome na sua própria categoria e garantindo que todos "
            "apareçam ao menos uma vez antes de repetir."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Caminho do CSV com as categorias. Padrão: nomes_exemplo.csv",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Quantidade de rodadas. Se não informar, usa o mínimo necessário "
            "para que todas as pessoas apareçam ao menos uma vez."
        ),
    )
